...
import net.mcreator.larnachianpantheon.procedures.TestProjectileWhileProjectileFlyingTickProcedure;
...
public class TestProjectileEntity extends AbstractArrow implements ItemSupplier {
	public static final ItemStack PROJECTILE_ITEM = new ItemStack(Blocks.ACACIA_SAPLING);

	public TestProjectileEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(LarnachianPantheonModEntities.TEST_PROJECTILE.get(), world);
		setNoGravity(true);
	}
	...
	public void tick() {
		super.tick();
		TestProjectileWhileProjectileFlyingTickProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this.getOwner(), this);
		if (this.inGround)
			this.discard();
	}

	public static TestProjectileEntity shoot(Level world, LivingEntity entity, RandomSource source) {
		return shoot(world, entity, source, 0.3f, 5, 5);
	}
	...
	public static TestProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		TestProjectileEntity entityarrow = new TestProjectileEntity(LarnachianPantheonModEntities.TEST_PROJECTILE.get(), entity, world);
	...
	public static TestProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		TestProjectileEntity entityarrow = new TestProjectileEntity(LarnachianPantheonModEntities.TEST_PROJECTILE.get(), entity, entity.level());
